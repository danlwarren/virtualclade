...
#' @param model "BM" for Brownian motion, "OU" for Ornstein-Uhlenbeck
#' @param alpha The alpha parameter for model = "OH", setting the strength of selective constraint.  Theta (the optimum for each parameter) is assumed to be the value at the root.
...
evolve.clade <- function(root.species, ntaxa, env, model = "BM", alpha = 0.1, rate = 0.2, min.sd = .5, tip.prefix = "species", max.tries = 10, ...){
  ...
  while(tries <= max.tries){
    for(i in names(root.params)){
      ...
      for(j in names(root.params[[i]]$args)){
        ...
        if(grepl("sd", j)){
          # Constraining the sd to be > a proportion of the root value
